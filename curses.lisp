...
(defpackage :curses
  (:use :common-lisp :cffi :trivial-gray-streams :cl-utilities)
  (:export :refresh :erase :move :*stdscr*
           ...
           :derwin :newwin :subwin :delwin :box :hline :vline
           ...
           :*ACS_LLCORNER* :*ACS_LRCORNER* :*ACS_URCORNER* :*ACS_ULCORNER*
           :*ACS_DEGREE*
           ))
...
(defvar *ACS_DEGREE* 186)
(defvar *ACS_BULLET* 102)
(defvar *ACS_URCORNER* 107)
(defvar *ACS_ULCORNER* 108)
(defvar *ACS_LRCORNER* 106)
(defvar *ACS_LLCORNER* 109)
...
(defcfun "wprintw" :int (wnd :pointer) (string :string))

(defun mvplines (y x str)
  (let ((lines (split-sequence #\linefeed str)))
    (loop for i from 0 upto (- (list-length lines) 1) do
          (mvprintw (+ y i) x (nth i lines)))))
...
(defcfun "box" :int (win :pointer) (verch chtype) (horch chtype))
